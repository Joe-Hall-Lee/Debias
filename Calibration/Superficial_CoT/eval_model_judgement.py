from judgelm.llm_judge.common import parse_score, translate_score_to_win_list, translate_score_to_win_list_reverse
...
def calculate_metrics(gt_answer_file_path, sequential_pred_answer_file_path, reversed_pred_answer_file_path, if_filter_minus_one=True):
    ...
    if isinstance(sequential_pred_answer_file_path, str):
        sequential_pred_answer_file_list = extract_jsonl(
            sequential_pred_answer_file_path)  # [:1000]
    ...
    y_true = gt_win_list
    y_true_reverse = translate_score_to_win_list_reverse(gt_score_list, T)
    ...
    if isinstance(reversed_pred_answer_file_path, str):
        reversed_pred_answer_file_list = extract_jsonl(
            reversed_pred_answer_file_path)
    ...
    for reversed_pred_answer_file in reversed_pred_answer_file_list:
        reversed_pred_score_list.append(parse_score(
            reversed_pred_answer_file['pred_text']))
    _, reversed_pred_score_list = filt_pred_score_list_by_gt_score_list(gt_score_list_contains_minus_one,
                                                                        reversed_pred_score_list)
    reversed_pred_win_list = translate_score_to_win_list(
        reversed_pred_score_list, T)

    metrics_dict['reverse_accuracy'] = accuracy_score(
        y_true_reverse, reversed_pred_win_list)
    metrics_dict['average_accuracy'] = (
        metrics_dict['reverse_accuracy'] + metrics_dict['accuracy']) / 2
    ...
    for i in range(len(sequential_pred_win_list)):
        ...
        if sequential_pred_win_list[i] == reversed_pred_win_list[i]:
            same += 1
        # 1 & 0， 1 & -1， 0 & -1
        elif sequential_pred_win_list[i] - reversed_pred_win_list[i] > 0:
            perfer_after += 1
            # print(i)
        # -1 & 0， -1 & 1， 0 & 1
        elif sequential_pred_win_list[i] - reversed_pred_win_list[i] < 0:
    ...
    metrics_dict['perfer_before_rate'] = perfer_before / \
        len(sequential_pred_win_list)
    metrics_dict['perfer_after_rate'] = perfer_after / \
        len(sequential_pred_win_list)
    metrics_dict['delta_bias'] = abs(
        perfer_before - perfer_after) / len(sequential_pred_win_list)
    metrics_dict['total_bias'] = (
        perfer_before + perfer_after) / len(sequential_pred_win_list)
...
if __name__ == '__main__':
    # gt w/o ref
    gt_answer_file_path = "F:/CS/AI/LLM-Evaluation/Dataset/LLMBar/Adversarial/GPTOut/converted_dataset_gpt4.jsonl"

    # 33b 100k full model lr 3e-5
    sequential_pred_answer_file_path = "F:/CS/AI/LLM-Evaluation/GPT4/Superficial_CoT/judgements_output/Adversarial/GPTOut/7b-full-model"
    #
    # 33b 100k full model lr 3e-5
    reversed_pred_answer_file_path = "F:/CS/AI/LLM-Evaluation/GPT4/Superficial_CoT/judgements_output/Adversarial/GPTOut/7b-full-model-reverse"

    parser = argparse.ArgumentParser()
    parser.add_argument("--gt-answer-file-path", type=str,
                        default=gt_answer_file_path)
    parser.add_argument("--sequential-pred-answer-file-path",
                        type=str, default=sequential_pred_answer_file_path)
    parser.add_argument("--reversed-pred-answer-file-path",
                        type=str, default=reversed_pred_answer_file_path)
